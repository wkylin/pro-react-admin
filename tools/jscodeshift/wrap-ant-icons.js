...
export default function transformer(file, api) {
  ...
  if (!hasAnimatedImport) {
    ...
    if (firstImport.size() > 0) {
      firstImport.insertBefore(j.importDeclaration([j.importDefaultSpecifier(j.identifier('AnimatedIcon'))], j.literal(ANIMATED_IMPORT)));
    } else {
      root.get().node.program.body.unshift(j.importDeclaration([j.importDefaultSpecifier(j.identifier('AnimatedIcon'))], j.literal(ANIMATED_IMPORT)));
    }
  }
