...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <code>validateChar</code>
      </td>
      <td>
        <code>(char: string) => boolean?</code>
      </td>
      <td>-</td>
      <td>
        字符验证函数。用于限制输入的字符类型。例如，<code>(ch) => /\d/.test(ch)</code> 只允许数字；
        <code>(ch) => ch === '' || /\d/.test(ch)</code>{' '}
        允许数字和删除（空字符）。如果未提供，默认允许任何单字符（包括空字符）。
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>'modern' | 'compact' | 'classic'?</code>
      </td>
      <td>
        <code>'modern'</code>
      </td>
      <td>
        视觉变体。<code>modern</code> 为现代样式（渐变背景、阴影）；<code>compact</code> 为紧凑样式（小尺寸、无阴影）；
        <code>classic</code> 为经典样式（简单边框、无渐变）。
      </td>
    </tr>
  </tbody>
</table>
