...
function Example() {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(!isOpen)}>{isOpen ? '关闭' : '打开'}</button>
      <InteractiveBook
        coverImage="https://example.com/cover.jpg"
        bookTitle="受控书籍"
        pages={pages}
...
function Example() {
  return (
    <>
      {/* 默认开启音效 */}
      <InteractiveBook coverImage="/cover.jpg" pages={pages} enableSound={true} />

      {/* 禁用音效 */}
      <InteractiveBook coverImage="/cover.jpg" pages={pages} enableSound={false} />

      {/* 自定义音效 */}
      <InteractiveBook coverImage="/cover.jpg" pages={pages} soundSrc="/audio/custom-flip.mp3" />
    </>
  )
}
```
...
```tsx
<InteractiveBook className="my-custom-book" coverImage="/cover.jpg" pages={pages} />
```
