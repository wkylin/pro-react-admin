...
const Sidebar = ({ config, setConfig, isDeploying, onStart, onReset, onCancel, errors = {}, isConfigValid }) => (
  <aside className={styles.sidebar}>
    <section>
      ...
      <div className={styles.formGroup}>
        <label>输出目录</label>
        <input type="text" value={config.distDir} onChange={(e) => setConfig({ ...config, distDir: e.target.value })} />
    ...
    <section>
      ...
      <div className={styles.actionsRow}>
        <Button
          ...
          size="middle"
          onClick={() => {
            if (!isDeploying) onStart && onStart()
          }}
        ...
        <Button
          size="middle"
          onClick={() => {
            if (!isDeploying) onReset && onReset()
          }}
          disabled={isDeploying}
        >
          重置
  ...
  return (
    <FixTabPanel fill={true}>
      <div className={styles.app}>
                  ...
                  {history.map((h) => (
                    <li key={h.jobId} className={styles.historyItem}>
                      ...
                      <div className={styles.historyActions}>
                        <Button size="small" onClick={() => viewHistoryLogs(h.jobId)} disabled={isDeploying}>
                          查看日志
                        </Button>
                        <Button size="small" onClick={() => retryDeployment(h.jobId)} disabled={isDeploying}>
                          重试
                        </Button>
                        {h.status === 'running' && (
                          <Button size="small" danger onClick={() => cancelDeployment(h.jobId)} disabled={!isDeploying}>
                            取消
                          </Button>
                        )}
                      </div>
        ...
        {toast.show && <div className={`${styles.toast} ${styles.show}`}>{toast.msg}</div>}
      </div>
    </FixTabPanel>
  )
}
