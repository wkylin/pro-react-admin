...
| `src/app-hooks`        | 与业务强相关的应用级 Hook（权限、数据拉取协议、Redux 等）。  | 通过相对路径引用，不走别名，可依赖 `permissionService` 等仅在应用内部存在的服务。                                                              |
...
- 依赖仅在应用层存在的服务（如 `permissionService`、自研 store、埋点）的 Hook，放在 `src/app-hooks`。
