...
// 下载（会触发浏览器下载）
request
  .download('/api/export', { q: 'all' }, 'report.xlsx', {
...
const results = await request.parallel(
  [['get', '/api/a'], ['post', '/api/b', { x: 1 }], () => request.get('/api/c')],
  /* concurrency */ 3
)
...
```js
// 依次执行
const seriesResults = await request.series([() => request.get('/api/step1'), ['post', '/api/step2', { id: 123 }]])
```
